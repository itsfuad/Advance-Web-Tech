...
import { Plus, Target, TrendingUp, Heart, BarChart3, Edit, Trash2 } from 'lucide-react';
...
export default function DashboardPage() {
  ...
  useEffect(() => {
    ...
    if (user) {
      api.get('/stats/me').then(res => setStats(res.data)).finally(() => setLoading(false));
    }
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-10">
        <Card>
          <CardContent className="p-5">
            ...
            <div className="text-2xl font-bold">{stats?.campaignsCreated ?? 0}</div>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="p-5">
            <div className="flex items-center gap-2 text-neutral-500 mb-1">
              <TrendingUp size={16} />
              <span className="text-xs uppercase tracking-wide">Total Raised</span>
            </div>
        ...
        <Card>
          <CardContent className="p-5">
            <div className="flex items-center gap-2 text-neutral-500 mb-1">
              <Heart size={16} />
              <span className="text-xs uppercase tracking-wide">Donations Made</span>
            </div>
            <div className="text-2xl font-bold">{stats?.donationsMade ?? 0}</div>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="p-5">
      ...
      <div className="mb-10">
        ...
        {!stats?.campaigns || stats.campaigns.length === 0 ? (
          <div className="border border-dashed border-neutral-200 rounded-lg p-10 text-center">
            ...
            <Link href="/campaigns/new">
              <Button className="mt-4" variant="outline">Create your first campaign</Button>
            </Link>
          </div>
        ) : (
          <div className="space-y-3">
            {stats.campaigns.map((campaign: Campaign) => (
              <div key={campaign.id} className="border border-neutral-200 rounded-lg p-4 flex items-center gap-4">
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1">
                    <Link href={`/campaigns/${campaign.id}`} className="font-medium hover:underline truncate">
                      {campaign.title}
                    </Link>
                    <Badge variant={campaign.status === 'active' ? 'success' : campaign.status === 'frozen' ? 'warning' : 'outline'}>
                      {campaign.status}
                    </Badge>
                  </div>
                  <div className="flex items-center gap-3">
                    <Progress value={getProgressPercentage(campaign.raisedAmount, campaign.goalAmount)} className="h-1.5 w-32" />
        ...
        <div>
          ...
          <div className="space-y-2">
            {stats.recentDonations.map((donation: Donation) => (
              <div key={donation.id} className="border border-neutral-200 rounded-lg p-4 flex items-center justify-between">
                <div>
                  <Link href={`/campaigns/${donation.campaignId}`} className="font-medium text-sm hover:underline">
                    {donation.campaign?.title}
                  </Link>
                  <p className="text-xs text-neutral-500 mt-0.5">{formatDate(donation.createdAt)}</p>
                </div>
