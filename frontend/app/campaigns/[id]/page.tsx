...
import { Target, Calendar, Flag, Edit, Trash2, CreditCard } from 'lucide-react';
...
export default function CampaignDetailPage() {
  ...
  const canDonate = user && !isOwner && campaign.status === 'active';
        ...
        <div className="lg:col-span-2">
          <div className="flex items-start justify-between gap-4 mb-4">
            <div>
              ...
              <p className="text-neutral-500 text-sm mt-1">by {campaign.creator?.name}</p>
            </div>
          ...
          <div className="prose max-w-none">
            <p className="text-neutral-700 whitespace-pre-wrap leading-relaxed">{campaign.description}</p>
          </div>
            ...
            <div className="mt-10">
              ...
              <div className="space-y-3">
                {donations.slice(0, 10).map((donation) => (
                  <div key={donation.id} className="flex items-center justify-between py-3 border-b border-neutral-100">
                    <div>
                      <p className="font-medium text-sm">{donation.donor?.name}</p>
                      {donation.message && (
                        <p className="text-xs text-neutral-500 mt-0.5">&ldquo;{donation.message}&rdquo;</p>
                      )}
                    </div>
                    <div className="text-right">
                      <p className="font-semibold">{formatCurrency(donation.amount)}</p>
                      <p className="text-xs text-neutral-400">{formatDate(donation.createdAt)}</p>
                    </div>
                  </div>
                ))}
              </div>
        ...
        <div className="lg:col-span-1">
          <Card className="sticky top-24">
            <CardContent className="p-6">
              <div className="text-3xl font-bold mb-1">{formatCurrency(campaign.raisedAmount)}</div>
              ...
              {!user && (
                <Link href="/login">
                  <Button className="w-full" size="lg">Sign in to Donate</Button>
                </Link>
              )}

              {isOwner && (
                <p className="text-center text-sm text-neutral-500">This is your campaign</p>
              )}
      ...
      <Dialog open={showDonateModal} onOpenChange={setShowDonateModal}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>Support &ldquo;{campaign.title}&rdquo;</DialogDescription>
          </DialogHeader>
          ...
          {donateSuccess ? (
            <div className="py-6 text-center">
              ...
              <p className="text-neutral-500 mt-1">Your donation was successful.</p>
              <Button className="mt-4 w-full" onClick={() => { setShowDonateModal(false); setDonateSuccess(false); }}>
                Close
              </Button>
            </div>
          ) : (
            <div className="space-y-4">
              ...
              <div className="flex gap-2">
                {[10, 25, 50, 100].map((amt) => (
                  <button
                    key={amt}
                    type="button"
                    onClick={() => setDonationForm(f => ({ ...f, amount: String(amt) }))}
                ...
                <div className="space-y-3">
                  <div className="space-y-1.5">
                    ...
                    <Input
                      ...
                      onChange={(e) => setDonationForm(f => ({ ...f, cardNumber: e.target.value }))}
                    />
                  </div>
                  <div className="grid grid-cols-2 gap-3">
                    <div className="space-y-1.5">
                      ...
                      <Input
                        ...
                        onChange={(e) => setDonationForm(f => ({ ...f, expiryDate: e.target.value }))}
                      />
                    </div>
                    <div className="space-y-1.5">
                      <Label>CVV</Label>
                      <Input
                        ...
                        onChange={(e) => setDonationForm(f => ({ ...f, cvv: e.target.value }))}
                      />
                    </div>
                  </div>
                </div>
              ...
              <DialogFooter>
                <Button variant="outline" onClick={() => setShowDonateModal(false)}>Cancel</Button>
                <Button
      ...
      <Dialog open={showReportModal} onOpenChange={setShowReportModal}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>Help us understand why this campaign should be reviewed.</DialogDescription>
          </DialogHeader>
          <div className="space-y-3">
            <Label>Reason</Label>
            <Textarea
          ...
          <DialogFooter>
            ...
            <Button variant="destructive" onClick={handleReport} loading={reportLoading}>Submit Report</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={showDeleteModal} onOpenChange={setShowDeleteModal}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Are you sure you want to delete &ldquo;{campaign.title}&rdquo;? This action cannot be undone.
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
